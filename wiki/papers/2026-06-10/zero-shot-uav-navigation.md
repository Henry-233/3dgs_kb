...
| 前作 | 区别 |
|------|------|
| [[papers/2026-05-28/gaussnav\|GaussNav]] (2025) | **导航范式不同**：GaussNav是模块化方法（探索→建图→匹配→规划），本工作是端到端RL（RGB→控制）；**场景不同**：GaussNav针对室内结构化环境（HM3D），本工作针对非结构化森林；**sim-to-real**：GaussNav纯仿真评估，本工作有真实飞行实验 |
| [[papers/2026-05-11/gs-livo\|GS-LIVO]] (2024) | GS-LIVO用Gaussian-SLAM做定位+路径规划，属模块化方法；本工作的端到端RL不依赖显式定位和地图 |
...
## 关联
- 基于: [[papers/2026-05-07/3d-gaussian-splatting]]（改造了原始3DGS的表示——将SH耦合颜色替换为物理材质属性+显式光照）
- 相关方法: [[papers/2026-05-28/gaussnav]]（3DGS用于视觉导航的模块化方法），[[papers/2026-05-11/gs-livo]]（Gaussian-SLAM+导航规划）
